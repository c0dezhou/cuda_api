...
TEST_F(CuMemTest, MemcpyDtoDAsync_BasicBehavior) {
    ...
    // Wait for the async operation to complete before launching another copy
    // operation
...
TEST_F(CuMemTest, AC_EG_MemcpyDtoDAsync_Max) {
    ...
    // Allocating slightly less than the total free memory to leave some room
...
TEST_F(CuMemTest, AC_OT_MemcpyDtoDAsync_RepeatedCalls) {
    ...
    // cuMemcpyDtoDAsync(d_dst + size / 2, d_src, size / 2 * sizeof(int),
    // stream);
    // // ddst {0,0,0,0,0,5,6,7,8,9}

    cuMemcpyDtoDAsync(d_dst, d_src, size / 2 * sizeof(int), stream);
    cuStreamSynchronize(stream);
    cuMemcpyDtoDAsync(d_dst + size / 2, d_src + size / 2,
                      size / 2 * sizeof(int), stream);
    ...
    // cuMemcpyDtoDAsync(d_dst + size / 2, d_src + size / 2, size / 2 *
    // sizeof(int), stream2);
